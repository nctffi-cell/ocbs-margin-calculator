...
Khác bản update_ref_prices.py ở root máy:
  - Tự chứa trong repo webapp/ (Actions chỉ thấy file trong repo).
  - Đọc webapp/stocks.json, ghi webapp/static/prices.json + webapp/docs/prices.json.
...
WEBAPP      = os.path.dirname(HERE)                       # …/webapp
STOCKS_JSON = os.path.join(WEBAPP, "stocks.json")
OUT_STATIC  = os.path.join(WEBAPP, "static", "prices.json")
OUT_DOCS    = os.path.join(WEBAPP, "docs",   "prices.json")
...
def main():
    ...
    for out in (OUT_STATIC, OUT_DOCS):
